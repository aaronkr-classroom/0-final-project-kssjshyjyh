...
module.exports = {
    index: (req, res, next) => {
      Book.find() // index 액션에서만 퀴리 실행
        .then((books) => {
          // 사용자 배열로 index 페이지 렌더링
          res.locals.books = books; // 응답상에서 사용자 데이터를 저장하고 다음 미들웨어 함수 호출
          next();
        })
        .catch((error) => {
    ...
    create: (req, res, next) => {
        ...
        let newBookparams = new Book(getBooksParams(req.body)); // Listing 22.3 (p. 328)
        
        Book.create(newBookparams)
            .then((books) => {
    ...
    show: (req, res, next) => {
      ...
      Book.findById(bookId) // ID로 사용자 찾기
        .then((books) => {
          res.locals.books = books; // 응답 객체를 통해 다음 믿들웨어 함수로 사용자 전달
          next();
        })
        .catch((error) => {
    ...
    update: (req, res, next) => {
        let bookId = req.params.id,
          bookParams = getBooksParams(req.body);
